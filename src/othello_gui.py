...
class OthelloGUI:
    def __init__(self, black_name="Human", white_name="Human"):
        ...
        # Create the OthelloGame gamestate here (drawn from the original othello game code)
        ...
        self._board = othello_models.GameBoard(
            self._game_state, GAME_WIDTH, GAME_HEIGHT, self._root_window
        )
        self._black_score = othello_models.Score(
            othello.BLACK, self._game_state, self._root_window
        )
        self._white_score = othello_models.Score(
            othello.WHITE, self._game_state, self._root_window
        )
    ...
    def _new_game(self) -> None:
        ...
        self._player_turn.update_turn(othello.BLACK)
    ...
    def _on_board_clicked(self, event: tkinter.Event) -> None:
        ...
        if self._game_state.get_turn() == othello.BLACK and self._black_name == "Human":
            ...
            self._game_state.get_turn() == othello.WHITE and self._white_name == "Human"
        ):
            move = self._convert_point_coord_to_move(event.x, event.y)
            row = move[0]
            col = move[1]
    ...
    def _play(self, row, col):
        try:
            ...
            if (
                self._game_state.get_turn() == othello.BLACK
                and self._black_name != "Human"
            ):
                self._player_turn.display_cheater(othello.BLACK, row, col)
            elif (
                self._game_state.get_turn() == othello.WHITE
                and self._white_name != "Human"
            ):
    ...
    def _play_ai(self):
        ...
        if self._game_state.get_turn() == othello.BLACK and self._black_ai is not None:
            ...
            self._game_state.get_turn() == othello.WHITE and self._white_ai is not None
        ):
...
if __name__ == "__main__":
    ...
    # For testing purposes (without having to click)
    # OthelloGUI(black_name="Random", white_name="Random").run_auto()
