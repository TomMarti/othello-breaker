...
class GameBoard:
    def __init__(
        self,
        game_state: othello.OthelloGame,
        game_width: float,
        game_height: float,
        root_window,
    ) -> None:
    ...
    def _redraw_lines(self) -> None:
        ...
        for row in range(1, self._rows):
            self._board.create_line(
                0, row * row_multiplier, self.get_board_width(), row * row_multiplier
            )

        # Draw the column lines next
        for col in range(1, self._cols):
            self._board.create_line(
                col * col_multiplier, 0, col * col_multiplier, self.get_board_height()
            )
...
class Score:
    def __init__(
        self, color: str, game_state: othello.OthelloGame, root_window
    ) -> None:
...
class Turn:
    def __init__(self, game_state: othello.OthelloGame, root_window) -> None:
        ...
        self._timer = (
            time.time()
        )  # playing time in seconds for the current move of the player
    ...
    def display_cheater(self, cheater: str, row: int, col: int) -> None:
        ...
        self._turn_label["text"] = (
            f"{PLAYERS[cheater]} cheated with row {row} and col {col}!"
        )

    def display_type_error(self, player: str, row: int, col: int) -> None:
        self._turn_label["text"] = (
            f"{PLAYERS[player]} gives invalid type. Row : {type(row)}, Col : {type(col)}"
        )
    ...
    def _turn_text(self) -> None:
        ...
        time_elapsed = (
            self._total_time_BLACK
            if self._player == othello.BLACK
            else self._total_time_WHITE
        )
    ...
    def _opposite_turn(self) -> None:
        ...
        return {othello.BLACK: othello.WHITE, othello.WHITE: othello.BLACK}[
            self._player
        ]
...
class OptionDialog:
    def __init__(
        self, current_rows, current_columns, current_black_name, current_white_name
    ):
        ...
        self._player_option_list = ["Human"] + [
            os.path.basename(path).split(".")[0] for path in glob.glob("ai/*.py")
        ]
        ...
        self._row_label = tkinter.Label(
            master=self._row_frame, text="Rows:", font=DIALOG_FONT
        )
        self._row_label.grid(row=0, column=0, sticky=tkinter.E, padx=10, pady=10)
        self._rows = tkinter.IntVar()
        self._rows.set(current_rows)
        self._row_option_menu = tkinter.OptionMenu(
            self._row_frame, self._rows, *self._row_column_option_list
        )
        ...
        self._column_label = tkinter.Label(
            master=self._column_frame, text="Columns:", font=DIALOG_FONT
        )
        ...
        self._column_option_menu = tkinter.OptionMenu(
            self._column_frame, self._columns, *self._row_column_option_list
        )
        self._column_option_menu.grid(
            row=0, column=1, sticky=tkinter.W, padx=10, pady=10
        )
        ...
        self._black_label = tkinter.Label(
            master=self._black_frame, text="Black :", font=DIALOG_FONT
        )
        self._black_label.grid(row=0, column=0, sticky=tkinter.E, padx=10, pady=10)
        self._black = tkinter.StringVar()
        self._black.set(current_black_name)
        self._black_option_menu = tkinter.OptionMenu(
            self._black_frame, self._black, *self._player_option_list
        )
        self._black_option_menu.grid(
            row=0, column=1, sticky=tkinter.W, padx=10, pady=10
        )
        self._black_frame.grid(row=1, column=0, sticky=tkinter.W, padx=10, pady=10)

        # white drop-down menu
        self._white_frame = tkinter.Frame(master=self._dialog_window)
        self._white_label = tkinter.Label(
            master=self._white_frame, text="White :", font=DIALOG_FONT
        )
        self._white_label.grid(row=0, column=0, sticky=tkinter.E, padx=10, pady=10)
        self._white = tkinter.StringVar()
        self._white.set(current_white_name)
        self._white_option_menu = tkinter.OptionMenu(
            self._white_frame, self._white, *self._player_option_list
        )
        self._white_option_menu.grid(
            row=0, column=1, sticky=tkinter.W, padx=10, pady=10
        )
        self._white_frame.grid(row=1, column=1, sticky=tkinter.W, padx=10, pady=10)
